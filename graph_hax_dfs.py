...
start with the all-zero bit string of size n
flip one bit, add to graph, and compute neighbors
only add to graph if neighbor doesn't exist, otherwise just add neighbor to node's neighbor list
eventually expand out to the entire cube!
...
n1 00, adj[] adj() --> check if 10 in graph -> no, new_node, add to graph, call adj()
n2 10, adj[n1] adj() --> check if 00 in graph (first bit flip) -> yes, add to adj list, then check 11 (second bit flip)
...
"""
	...
	if not start in map(lambda node: node.bitstr, self.nodes):
		self.nodes.append(Node(start, self))
		self.nodes[-1].adjacents(self)
	...
	def helper(start, dimension):
		...
		helper(start + '0', dimension-1)
		helper(start + '1', dimension-1)
...
"""
		[node: 100, neighbors: ['000', '110', '101'], cheapest_hamming: 000, 
		node: 000, neighbors: ['100', '010', '001'], cheapest_hamming: 100, 
		node: 110, neighbors: ['010', '100', '111'], cheapest_hamming: 010]	

		chp = cheapest_hamming(start_node.adjacent)[0]
		if chp not in path:
			path.append(chp)

		adj_copy = start_node.adjacent[:]
		adj_copy.remove(chp)
		path.append(cheapest_hamming(adj_copy)[0])

		start_node = path[-1]

		
		why is code above faster?
		...
		while chp in path:
			adj_copy = start_node.adjacent[:]
			adj_copy.remove(chp)
			chp = cheapest_hamming(adj_copy)[0]
...
# how to prevent from being stuck in cycle {i.e. 100 --> 000 --> 100}, end goal: 110 ?
				# we can't go back to a node that's in path... so if it is select next lower hamming 
				# must compute next cheapest hamming
